...
Rewriting an already-sent user message. The `thread.message.edit` command in [the contracts][1] carries the new text plus a _resolution_ saying what should happen to the work the agent already did after that message: `fork`, `rewind`, or `continue`. [MessageEditReactor.ts][25] carries the resolution out; file state always comes from the checkpoint system and agent context always comes from provider-native fork/rollback primitives.

#### Fork thread

A thread created by copying another thread's transcript up to a _fork point_, recorded on the new thread as `forkedFromThreadId`, `forkPointMessageId`, and `forkKind`. Copies get fresh message ids — message ids are globally unique in the projection, so reusing them would move the rows off the source thread. Two kinds exist: `user` (a new direction taken from an edit point) and `archive-tail`.

#### Archive tail

The `archive-tail` fork a rewind leaves behind: a settled thread holding every turn the rewind discarded, so no work is lost. The source thread gets a `thread.tail-archived` activity linking to it.
...
[24]: ./overview.md
[25]: ../../apps/server/src/orchestration/Layers/MessageEditReactor.ts
