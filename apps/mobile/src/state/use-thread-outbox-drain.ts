  ...
  useEffect(() => {
    ...
    for (const [threadKey, queuedMessages] of Object.entries(queuedMessagesByThreadKey)) {
      ...
      const delivery = confirmThreadOutboxMessageQueued(nextQueuedMessage).then((queued) => {
        ...
        // created target defers, a thread that started working since the
        // guards ran stays queued in queue mode, and a steer-mode message
        // only dispatches once its turn is actually running.
        if (deliveryAction === "send") {
          ...
          const liveThreadSteerable = liveThread?.session?.status === "running";
          const liveDeliveryAction = resolveThreadOutboxDeliveryAction({
            ...
            threadSteerable: liveThreadSteerable,
            activeTurnMessageBehavior: nextQueuedMessage.activeTurnMessageBehavior,
            held: Boolean(heldThreadKeys[threadKey]),
          });
          if (
            liveDeliveryAction !== "send" ||
            shouldDeferConfirmedThreadOutboxDelivery({
              deliveryAction,
              isCreation: creation !== undefined,
              threadBusy: liveThreadBusy,
