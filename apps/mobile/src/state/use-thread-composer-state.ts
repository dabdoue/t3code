...
export function useThreadComposerState() {
  ...
  const activeThreadComposerThread = selectedThreadDetail ?? selectedThreadShell;
  const activeThreadBusy =
    !!activeThreadComposerThread &&
    (activeThreadComposerThread.session?.status === "running" ||
      activeThreadComposerThread.session?.status === "starting");
