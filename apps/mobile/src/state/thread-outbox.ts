...
// Removal (including the environment-wide clear) lives in
// `thread-outbox-removal.ts`: taking a message out of the outbox must also
// release its local attachment files, and that owner needs the composer draft
// state this module must not depend on.
