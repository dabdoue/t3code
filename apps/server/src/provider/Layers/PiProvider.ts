...
/**
 * Product slug, not a pi model id — the adapter treats it as "pi's own default
 * model" and never sends it to `set_model`. Real models come from pi's own
 * provider registry (including locally declared ones in `~/.pi/agent/models.json`).
 */
const PI_DEFAULT_MODEL_SLUG = "pi-default";
...
/**
 * pi is file-configured (agents.json / models.json, often pointed at a local
 * or remote OpenAI-compatible server), so there is no login probe: the health
 * check is `pi --version`, and readiness means the binary answers.
 */
