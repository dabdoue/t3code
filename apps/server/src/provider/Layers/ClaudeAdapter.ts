...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleSdkMessage = Effect.fn("handleSdkMessage")(function* (
    ...
    // Wire-only command bookkeeping has no user-facing T3 lifecycle. Newer
    // Claude Code CLIs (2.1.x) emit this telemetry, which this build's SDK
    // typings (0.3.170) predate; `sdkMessageType` reads it off the wire
    // without the exhaustiveness switch below surfacing a runtime warning.
