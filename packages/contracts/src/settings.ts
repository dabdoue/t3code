...
export const ClaudeSettings = makeProviderSettingsSchema(
  {
    ...
    disable1mContext: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({
        title: "Disable 1M context",
        description: "Ask Claude Code to use standard context variants for this provider instance.",
      }),
    ),
  },
  {
    order: ["binaryPath", "homePath", "autoCompactWindow", "disable1mContext", "launchArgs"],
  },
);
...
export const PiAgentSettings = makeProviderSettingsSchema(
  {
    // On by default: pi instances only exist through an explicit
    // `providerInstances` entry, so an instance the user added should probe
    // and connect without a second settings visit.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    ...
    launchArgs: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "e.g. --provider my-vllm",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
const ClaudeSettingsPatch = Schema.Struct({
  ...
  disable1mContext: Schema.optionalKey(Schema.Boolean),
});
